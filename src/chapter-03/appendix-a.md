...
**图 A.1 理想 NPN 型晶体管的 Ebers-Moll Model**
...
混合 π 型模式，集电极电流可认为被有效基极-发射极间电压控制，所以在集电极-发射极间有从属电源 VCCS。VCCS 的电导，即相互电导（跨导）\\(g_m\\)，由第 2 章说明过的式（2.1）对 \\(V_{BE}\\) 求微分，并依次决定：
...
**图 A.5 基极-集电极间反向偏压 \\(V_{CB}\\) 增加时，基极宽就减少**
...
**图 A.6 \\(I_C\\)-\\(V_{CE}\\) 特性与 Early 电压的关系**
...
**图 A.8 \\(V_{BE}\\) 变化时，中性基极领域的少数蓄积电荷量与多数载波蓄积电荷量只做同样大小的变化**
...
**图 A.10 使用 Hybrid π 型模式以利于计算电流放大倍数相对频率的特性**
...
**图 A.11 \\(C_b\\)、\\(C_{JC}\\)、\\(C_{JE}\\) 相对集电极电流 \\(I_C\\) 的特性**
...
**图 A.12 暂态频率 \\(f_T\\) 对集电极电流 \\(I_C\\) 特性的一般倾向**

![图 A.13 东芝 2SC1815 ① 的 \\(f_T\\)-\\(I_C\\) 特性](images/fig-a-13.png)

**图 A.13 东芝 2SC1815 ① 的 \\(f_T\\)-\\(I_C\\) 特性**
