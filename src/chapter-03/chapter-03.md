...
式中，\\(r_\pi\\) 为 \\(Q_4\\) 的基极—发射极间电阻（\\(=1/g_\pi\\)），\\(r_x\\) 为 \\(Q_4\\) 的基极扩展电阻（\\(=1/g_x\\)），\\(g_m\\) 为\\(Q_4\\) 的互导，\\(r_o\\) 为\\(Q_4\\) 的输出电阻（\\(=1/g_o\\)），\\(R_{\mathrm{IN}}\\) 为输出级发射极跟随器的输入电阻。

\\(R_{\mathrm{IN}}\\) 约等于 \\(Q_5\\)、\\(Q_7\\) 的 \\(h_{fe}\\) 与输出级负载电阻的乘积。以图 3.5所示电路而言，应为：
...
**图 3.12　在七晶体管运算放大器添加 \\(Q_8\\)、\\(Q_9\\)、\\(Q_{10}\\) 的十晶体管正向放大器**
...
米勒效应，是 J. M. Miller 发现有关三极真空管屏极—栅极间电容的理论<sup>[1]</sup>。米勒效应不只限于真空管，凡是晶体管与 FET，还有图 3.21 所示一般的反向放大器均其作用。在图 3.21 中，若以 \\(A\\) 表示开环增益，则
