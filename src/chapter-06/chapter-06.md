...
\(Q_5\) 属于横向 PNP。从而 \(h_{\mathrm{FE}}\) 相当小，因此 \(D_4\) 与 \(Q_8\) 的电流镜像匹配的并不好，以致 \(I_{C5}\) 与 \(I_{\mathrm{B}1}\) 之间有 5%～20% 的误差[15,3]。假设此误差为 5%，则输入偏置电流约为 2 nA（参照图 6.3）。
...
实际上如图 6.5 所示，在 \(R_{24}\) 设置分接头（Tap），当 Wafer test 时把适当的邻接 Tap 加以短路，以调整 \(R_R/R_I\)[16]。借助短路调整电阻值的方法称为"zener zap trimming"。是在 Tap 间所连接的齐纳二极管（其实是 NPN 晶体管的基极-发射极结）中通以巨大的脉冲进发电流，并熔融短路齐纳二极管的方法。
...
![图 6.8　NJM4558(通用型) 与 OP07(高精度型) 的输入换算噪声电压相对频率的特性](images/fig-08.png)

![图 6.9　DC 伺服平坦放大器](images/fig-09.png)
...
![图 6.10　DC 伺服放大器的低频带频率特性](images/fig-10.png)
...
设若 \(Q_1\)、\(Q_2\) 的各互导为 \(g_m\)，当供加差动输入电压 \(v_{in}\) 时，\(Q_4\) 的集电极电流只减少 \((g_m/2)v_{in}\)。又 \(Q_4\) 的集电极电流只增加 \((g_m/2)v_n\)，B 点的电位上升。
