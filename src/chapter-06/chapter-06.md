...
在此根据第 2 章的式(2.1)与式(2.2)跨导 \\(g_m\\) 应为：
...
**表 6.2　具代表性的低噪声运算放大器的电气特性（* 记号表双运放，其他一律为单运放）**
...
![图 6.22　AD797 的简略等效电路](images/fig-06-22.svg)
...
设若 \\(Q_1\\)、\\(Q_2\\) 的各跨导为 \\(g_m\\)，当供加差动输入电压 \\(v_{in}\\) 时，\\(Q_3\\) 的集电极电流只减少 \\((g_m/2)v_{in}\\)。又 \\(Q_4\\) 的集电极电流只增加 \\((g_m/2)v_n\\)，B 点的电位上升。
...
![图 6.24　AD797 的开环增益、相位容限相对频率的特性](images/fig-06-24.svg)

**图 6.24　AD797 的开环增益、相位容限相对频率的特性**

![图 6.25　连接 \\(C_N\\) 时的等效电路](images/fig-06-25.svg)
