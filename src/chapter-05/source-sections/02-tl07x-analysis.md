...
\\(R_1\\) 与 \\(R_2\\) 的噪声有关连，原因是在一般情形下，FET 的 \\(g_m\\) 小于 BJT 的 \\(g_m\\)，而且输入型运算放大器不能忽视由初级有源负载的电流镜像所发生的噪声的缘故。\\(R_1\\) 与 \\(R_2\\) 的值增加时，\\(R_1\\) 与 \\(R_2\\) 的热噪声就会增加（后述），但因锁于 \\(Q_2\\) 的局部电流反馈量增加的更多，所以出现于 \\(Q_2\\) 集电极的噪声该减少<sup>[1]</sup>。
