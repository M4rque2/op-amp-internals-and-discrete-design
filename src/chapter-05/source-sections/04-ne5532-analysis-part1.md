...
此运算放大器属于 Signetics 公司，该公司随后被飞利浦公司所收购。但是，具有相同型号的运算放大器（Second Source）已由各公司所提供。

初级差动元件使用 NPN 型 BJT 的通用运算放大器。输入噪声级小到 5 \\(nV/\\sqrt{\\mathrm{Hz}}\\)，归类于低噪声类型并不见怪。在电源电压=±18 V，600Ω 的负载下可得 10V_RMS 的输出电压。
