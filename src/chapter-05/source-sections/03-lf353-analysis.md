...
④ 称为"Wilson 型电流镜像"<sup>[3]</sup>。图 5.8 的 \\(Q_{14}\\) 的集电极电流从属于 \\(Q_{16}\\) 的集电极电流。由于 Wilson 型镜加有多量的负反馈，所以 \\(Q_{14}\\) 的输出电阻相对较高（为发射极接地输出电阻的 (\\(h_{FE}\\)/2) 倍），而且电流镜像的电流放大倍数(\\(I_{C14}\\)/\\(I_{C16}\\)) 有极为接近于 1 的优点。
