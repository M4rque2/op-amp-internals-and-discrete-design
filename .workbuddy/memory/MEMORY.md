...
## Goal
Convert scanned PDF book 《电子元器件应用技术 基于OP放大器与晶体管的放大电路设计》(黑田徹) into mdbook-compatible markdown. **ALL 8 CHAPTERS DONE (book complete, PDF = 235 pp).**
...
- Chapter boundary mapping (PDF page : original book page):
  - Ch1: ~p14–? ; Ch2: ~p? ; Ch3: ? ; Ch4: PDF 113–150 (+附录C) ; Ch5: PDF 151–170 (orig 137–156) ; Ch6: PDF 171–192 ; Ch7: PDF 193–218 (orig 177–204) ; **Ch8: PDF 219–235 (orig 205–221), 20 figs + 6 tables + 1 photo + 9 eqs — LAST PAGE OF BOOK**. No appendices after Ch8.
...
- **Never use `\{` or `\}` inside math** — CommonMark consumes the backslash before MathJax sees it (`\left\{` breaks). Use `\lbrace`/`\rbrace` instead (fixed in eq 7.25). `\left(` is fine (`(` is plain).
